  ...
  [5362],
  {
    63993: (e, t, a) => {
      ...
      async function p(e, t) {
        const a = await fetch(e, { credentials: 'same-origin' }).finally(() => {
          t?.();
        });
        return { isFullyAvailable: !0, fullDocument: await a.arrayBuffer() };
      }
      var g = a(33075);
      let y;
      class w extends l.Ay {
        constructor(e) {
          const t = e.baseUrl || (0, o.$_)(window.document),
            a = e.baseCoreUrl || t,
            s = e.baseProcessorEngineUrl || t,
            n = { ...e, baseUrl: t, baseCoreUrl: a, baseProcessorEngineUrl: s };
          if ('string' != typeof n.baseUrl)
            throw new i.uE(
              '`baseUrl` is mandatory and must be a valid URL, e.g. `https://example.com/`'
            );
          if (
            'string' != typeof n.document &&
            !(n.document instanceof ArrayBuffer)
          )
            throw new i.uE(
          ...
          if (
            'string' == typeof n.licenseKey &&
            n.licenseKey.startsWith('TRIAL-')
          )
            throw new i.uE(
        ...
        async load() {
          ...
          const a = await _(this.client, this._state).finally(() => {
            ...
            n = (async function (e, t, a, s) {
              if (e instanceof ArrayBuffer)
                return { isFullyAvailable: !0, fullDocument: e };
              (0, i.V1)('string' == typeof e, 'Document is not a string');
              let n = !1;
              const { linearizedInstance: r, serverSupportsRangeRequests: o } =
                await (async () => {
                  if (a) {
                    const a = t.initializeLinearizedInstance();
                    return {
                      linearizedInstance: a,
                      serverSupportsRangeRequests: await a.initialize(e),
                    };
                  }
                  return {
                    linearizedInstance: null,
                    serverSupportsRangeRequests: !1,
                  };
                })();
              try {
                ...
                return t
                  ? (s?.(), { isFullyAvailable: !1, documentResponse: t })
                  : (r.destroy(), (n = !0), p(e, s));
              } catch (t) {
                if ((r?.destroy(), n)) throw t;
                return p(e, s);
              }
            })(
              this._state.document,
          ...
          const {
            features: o,
            signatureFeatureAvailability: l,
            capabilities: c,
          } = a;
          if (
            this._state.productId === h.v.SharePoint &&
            'string' == typeof this._state.document &&
            Array.isArray(a.afu)
          ) {
            ...
            if (!a.afu.some((t) => e.hostname.match(t)))
              throw new i.uE(
                `The document origin ${e.hostname} is not authorized.`
              );
          }
          const f =
            l === u.g.ELECTRONIC_SIGNATURES &&
            (0, d.UX)(o) &&
          ...
          if (w.isFullyAvailable) {
            let t = w.fullDocument.slice(0);
            try {
              this.destroyed
                ? ((t = null), (b = await new Promise(() => {})))
                : ((b = await this.client.openDocument(w.fullDocument, {
                    password: e.password,
                    initialPageIndex:
                      'number' == typeof e.initialPageIndex
                        ? e.initialPageIndex
                        : 0,
              ...
              if (
                ('INVALID_PASSWORD' === a.message &&
                  this._state.document instanceof ArrayBuffer &&
                  (this._state = this._state.set('document', a.callArgs[0])),
                'IMAGE_DOCUMENTS_NOT_LICENSED' === a.message &&
                  (a.message =
                ...
                !(
                  c?.includes('GdPictureWASM') &&
                  a instanceof i.uE &&
                  a.message.includes('File not in PDF format or corrupted.') &&
                  this._state.productId !== h.v.Salesforce
                ))
              )
                throw a;
              {
          ...
          return (
            (this._state = this._state.set('documentResponse', b)),
            {
              ...
              linearizedLoading: !w.isFullyAvailable,
              documentResponse: b,
            }
          );
        }
        ...
        async afterDocumentLoaded(e) {
          if (
            ...
            this._instantJSON && this._instantJSON.pdfId && e.ID.permanent)
          ) {
            const t = this._instantJSON.pdfId,
              a = e.ID;
            if (t.permanent !== a.permanent)
              throw new i.uE(
          ...
          if (this._trustedCAsCallback)
            try {
              ...
              if (!Array.isArray(e))
                throw new i.uE('Certificates response must be an array');
              if (
                e.some(
                  (e) => !(e instanceof ArrayBuffer) && 'string' != typeof e
                )
              )
                throw new i.uE(
                  'All certificates must be passed as ArrayBuffer (DER) or string (PEM)'
                );
              await this.client.loadCertificates(e.map(c.PI));
            } catch (e) {
              throw new i.uE(
        ...
        async waitUntilFullyLoaded(e) {
          (0, i.V1)(
            this.corePDFBridge.linearizedInstance,
      ...
      const b = { current: void 0 };
      async function _(e, t) {
        b.current =
          b.current || (t.customFonts ? (0, f.eY)(t.customFonts) : void 0);
        const a = (0, o.f)(t.appName);
        return e
          .loadNativeModule(t.baseCoreUrl, {
            ...
            workerSpawnerFn: () => (0, n.pj)(t.inlineWorkers),
          })
          .then(async () =>
            e.load(t.baseCoreUrl, t.licenseKey, {
              mainThreadOrigin: a,
              ...(b.current ? { customFonts: await b.current } : null),
