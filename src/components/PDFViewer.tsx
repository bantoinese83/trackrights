...
export default function PDFViewer({ pdfData }: PDFViewerProps) {
  ...
  const [pageNumber, setPageNumber] = useLocalStorage<number>(
    'trackrights_pdf_page',
    1
  );
  const [scale, setScale] = useLocalStorage<number>('trackrights_pdf_scale', 1);
  const [rotation, setRotation] = useLocalStorage<number>(
    'trackrights_pdf_rotation',
    0
  );
  const [isFullscreen, setIsFullscreen] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [searchText, setSearchText] = useLocalStorage<string>(
    'trackrights_pdf_search',
    ''
  );
